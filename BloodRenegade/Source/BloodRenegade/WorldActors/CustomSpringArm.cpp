...
UCustomSpringArm::UCustomSpringArm()
{
	bTickInEditor = false;
}

void UCustomSpringArm::BeginPlay()
{
	Super::BeginPlay();
	
	ActorOwner = GetOwner<ABloodCharacter>();
	CurrentPlayerState = GetOwner<ABloodCharacter>()->GetPlayerState<ABloodPlayerState>();
	InitialCamOffset = SocketOffset;
	CurveLocalMove = NewObject<UCurveFloat>();
	CurveLocalMove->FloatCurve.AddKey(0,0);
	CurveLocalMove->FloatCurve.AddKey(0.2f,1);
	
	FOnTimelineFloat MoveCamCallBack;
	MoveCamCallBack.BindUFunction(this, FName("LerpLocalCamNextPos"));
	MoveCamLocalTimeline.AddInterpFloat(CurveLocalMove, MoveCamCallBack);
}

void UCustomSpringArm::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	MoveCamLocalTimeline.TickTimeline(DeltaTime);
	UpdateDesiredArmLocation(true, bEnableCameraLag, bEnableCameraRotationLag, DeltaTime);

	if(!CurrentPlayerState->IsAiming && !CurrentPlayerState->IsDashing && !CurrentPlayerState->IsInNightmareForm)
	{
		DistClampBySpeed = FMath::GetMappedRangeValueClamped(FVector2d(0,DataAsset->WalkSpeed), FVector2d(170,370), UKismetMathLibrary::VSizeXY(ActorOwner->GetMovementComponent()->Velocity));
	}

	TargetArmLength = FMath::Lerp(TargetArmLength, DistClampBySpeed, 0.03f);
}

void UCustomSpringArm::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
...
void UCustomSpringArm::EnableAimCamPosition(float dir)
{
	SetCameraArmDist(170);
	NextLocalCamOffset = InitialCamOffset + FVector::RightVector*AimCamOffset*dir;
	MoveCamLocalTimeline.Play();
}

void UCustomSpringArm::DisableAimCamPosition()
{
	if(!CurrentPlayerState->IsAiming && ActorOwner->CanMoveWhileHiding && !CurrentPlayerState->IsInNightmareForm)
	{
		MoveCamLocalTimeline.Reverse();
	}
}


void UCustomSpringArm::LerpLocalCamNextPos(float value)
{
	SocketOffset = (FMath::Lerp(InitialCamOffset, NextLocalCamOffset,value));
}

void UCustomSpringArm::SetCameraArmDist(float d)
{
	DistClampBySpeed = d;
}
