...
AGOAPController::AGOAPController(const class FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) //.SetDefaultSubobjectClass<UCrowdFollowingComponent>(TEXT("PathFollowingComponent")))
{
	...
	bWantsPlayerState = false;										//The character gets an ID
}
...
void AGOAPController::SetActionCost(const FName* ActionName, int Cost)
{
...
void AGOAPController::GetActualWorldstate(GOAP::worldstate_t* ActualWorldStateWS)
{
...
void AGOAPController::SetActualWorldStateAtomValue(const char* AtomName, bool AtomValue)
{
	...
	FName atomKey = FName(AtomName);
	BlackboardComp->SetValueAsBool(atomKey, AtomValue);
}
...
bool AGOAPController::CheckGoal()
{
	...
	if (GOAPCharacter)
	{
		...
		GOAPCharacter->CheckGoals();
	}
	return true;
}

void AGOAPController::StopBehaviorTree() 
{
	
	BehaviorComp->StopTree();
	//BehaviorComp->PauseLogic("blind");
	
}

void AGOAPController::RestartBehaviorTree() 
{
	BehaviorComp->RestartTree();
	//BehaviorComp->RestartLogic();
}

ETeamAttitude::Type AGOAPController::GetTeamAttitudeTowards(const AActor& other) const 
{	
