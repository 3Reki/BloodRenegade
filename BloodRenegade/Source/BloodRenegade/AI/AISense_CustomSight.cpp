...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AISense_CustomSight)
...
// FAISightTarget
//----------------------------------------------------------------------//
const FAICustomSightTarget::FTargetId FAICustomSightTarget::InvalidTargetId = FAISystem::InvalidUnsignedID;

FAICustomSightTarget::FAICustomSightTarget(AActor* InTarget, FGenericTeamId InTeamId)
...
//----------------------------------------------------------------------//
UAISense_CustomSight::FDigestedSightProperties::FDigestedSightProperties(const UAISenseConfig_CustomSight& SenseConfig)
{
	SightRadiusSq = FMath::Square(SenseConfig.SightRadius + SenseConfig.PointOfViewBackwardOffset);
	ShootRadiusSq = FMath::Square(SenseConfig.ShootRadius + PointOfViewBackwardOffset);
	CloseSightRadiusSq = FMath::Square(SenseConfig.CloseSightRadius + SenseConfig.PointOfViewBackwardOffset);
	...
	TopClamp = SenseConfig.TopClamp;
	BottomClamp = SenseConfig.BottomClamp;
...
UAISense_CustomSight::FDigestedSightProperties::FDigestedSightProperties()
...
// UAISense_Sight
//----------------------------------------------------------------------//
UAISense_CustomSight::UAISense_CustomSight(const FObjectInitializer& ObjectInitializer)
	...
	, bUseAsynchronousTraceForDefaultSightQueries(bDefaultUseAsynchronousTraceForDefaultSightQueries)
{
	...
	//{
		UAISenseConfig_CustomSight* SightConfigCDO = GetMutableDefault<UAISenseConfig_CustomSight>();
		SightConfigCDO->Implementation = UAISense_CustomSight::StaticClass();

		OnNewListenerDelegate.BindUObject(this, &UAISense_CustomSight::OnNewListenerImpl);
		OnListenerUpdateDelegate.BindUObject(this, &UAISense_CustomSight::OnListenerUpdateImpl);
		OnListenerRemovedDelegate.BindUObject(this, &UAISense_CustomSight::OnListenerRemovedImpl);

		//OnPendingCanBeSeenQueryProcessedDelegate.BindUObject(this, &UAISense_CustomSight::OnPendingCanBeSeenQueryProcessed);
		OnPendingTraceQueryProcessedDelegate.BindUObject(this, &UAISense_CustomSight::OnPendingTraceQueryProcessed);
	//}
...
/*UAISense_CustomSight::UAISense_CustomSight()
{
	//Super();
	//Super::SetSenseID(FAISenseID::InvalidID());
	MaxTracesPerTick = DefaultMaxTracesPerTick;
	MaxAsyncTracesPerTick = DefaultMaxAsyncTracesPerTick;
	MinQueriesPerTimeSliceCheck = DefaultMinQueriesPerTimeSliceCheck;
...
FORCEINLINE_DEBUGGABLE float UAISense_CustomSight::CalcQueryImportance(const FPerceptionListener& Listener, const FVector& TargetLocation, const float SightRadiusSq) const
{
...
void UAISense_CustomSight::PostInitProperties()
{
	Super::PostInitProperties();
	HighImportanceDistanceSquare = FMath::Square(HighImportanceQueryDistanceThreshold);
}

bool UAISense_CustomSight::ShouldAutomaticallySeeTarget(const FDigestedSightProperties& PropDigest, FAICustomSightQuery* SightQuery, FPerceptionListener& Listener, AActor* TargetActor, float& OutStimulusStrength) const
{
...
namespace UE::UAISense_CustomSight
{
...
float UAISense_CustomSight::Update()
{
	...
	// sort Sight Queries
	{
		auto RecalcScore = [](FAICustomSightQuery& SightQuery)->EForEachResult
			{
		...
		if (bSightQueriesOutOfRangeDirty)
		{
			...
			SightQueriesOutOfRange.Sort(FAICustomSightQuery::FSortPredicate());
		...
		SightQueriesInRange.Sort(FAICustomSightQuery::FSortPredicate());
	}
	...
	TArray<FAICustomSightTarget::FTargetId> InvalidTargets;
	...
	for (int32 QueryIndex = 0; QueryIndex < SightQueriesInRange.Num() + SightQueriesOutOfRange.Num(); ++QueryIndex)
	{
		...
		FAICustomSightQuery* InRangeQuery = InRangeIndex != INDEX_NONE ? &SightQueriesInRange[InRangeIndex] : nullptr;
		...
		FAICustomSightQuery* OutOfRangeQuery = OutOfRangeIndex != INDEX_NONE ? &SightQueriesOutOfRange[OutOfRangeIndex] : nullptr;
		if (OutOfRangeQuery)
		{
		...
		const bool bIsInRangeQuery = (InRangeQuery && OutOfRangeQuery) ? FAICustomSightQuery::FSortPredicate()(*InRangeQuery, *OutOfRangeQuery) : !OutOfRangeQuery;
		FAICustomSightQuery* SightQuery = bIsInRangeQuery ? InRangeQuery : OutOfRangeQuery;
		ensure(SightQuery);
		...
		FAICustomSightTarget& Target = ObservedTargets[SightQuery->TargetId];
	...
	if (QueryOperations.Num() > 0)
	{
		...
		TArray<FAICustomSightQuery> SightQueriesOutOfRangeToInsert;
...
bool CheckIsTargetInSightConeCustom(const FVector& StartLocation, const FVector& ConeDirectionNormal, float PeripheralVisionAngleCos,
	float ConeDirectionBackwardOffset, float NearClippingRadiusSq, float const FarClippingRadiusSq, float const ShootRadiusSq, float CloseSightRadiusSq, float const TopClamp, float const BottomClamp, const FVector& TargetLocation)
{
	...
	const float DeltaHeight = (TargetLocation - StartLocation).Z;
	if (DistToTargetSq <= FarClippingRadiusSq && DistToTargetSq >= NearClippingRadiusSq && ((DistToTargetSq >= ShootRadiusSq && DeltaHeight > BottomClamp && DeltaHeight < TopClamp) || (DistToTargetSq < ShootRadiusSq)))
	{
		// Will return true if squared distance to Target is smaller than SMALL_NUMBER
		if (DistToTargetSq < (CloseSightRadiusSq))
		{
			return true;
		}
...
UAISense_CustomSight::EVisibilityResult UAISense_CustomSight::ComputeVisibility(UWorld* World, FAICustomSightQuery& SightQuery, FPerceptionListener& Listener, const AActor* ListenerActor, FAICustomSightTarget& Target, AActor* TargetActor, const FDigestedSightProperties& PropDigest, float& OutStimulusStrength, FVector& OutSeenLocation, int32& OutNumberOfLoSChecksPerformed, int32& OutNumberOfAsyncLosCheckRequested) const
{
	...
	if (!CheckIsTargetInSightConeCustom(Listener.CachedLocation, Listener.CachedDirection, PropDigest.PeripheralVisionAngleCos, PropDigest.PointOfViewBackwardOffset, PropDigest.NearClippingRadiusSq, SightRadiusSq, PropDigest.ShootRadiusSq, PropDigest.CloseSightRadiusSq, PropDigest.TopClamp, PropDigest.BottomClamp, TargetLocation))
	{
	...
	{
		ABloodCharacter* BloodCharacter = Cast<ABloodCharacter>(TargetActor);
		FHitResult HitResult;
		bool bHit;
		if (BloodCharacter != nullptr && BloodCharacter->VisiblePoint != nullptr)
		{
			bHit = World->LineTraceSingleByChannel(HitResult, Listener.CachedLocation, BloodCharacter->VisiblePoint->GetComponentLocation(), DefaultSightCollisionChannel, QueryParams, FCollisionResponseParams::DefaultResponseParam);
			//DrawDebugLine(GetWorld(), Listener.CachedLocation, BloodCharacter->VisiblePoint->GetComponentLocation(), FColor::Emerald);
		}
		else 
		{
			bHit = World->LineTraceSingleByChannel(HitResult, Listener.CachedLocation, TargetLocation, DefaultSightCollisionChannel, QueryParams, FCollisionResponseParams::DefaultResponseParam);
		}
		...
		if (UE::UAISense_CustomSight::IsTraceConsideredVisible(bHit ? &HitResult : nullptr, TargetActor))
		{
...
void UAISense_CustomSight::UpdateQueryVisibilityStatus(FAICustomSightQuery& SightQuery, FPerceptionListener& Listener, const bool bIsVisible, const FVector& SeenLocation, const float StimulusStrength, AActor* TargetActor, const FVector& TargetLocation) const
{
	if (bIsVisible)
	{
...
void UAISense_CustomSight::OnPendingCanBeSeenQueryProcessed(const FAICustomSightQueryID& QueryID, const bool bIsVisible, const float StimulusStrength, const FVector& SeenLocation, const TOptional<int32>& UserData)
{
	SCOPE_CYCLE_COUNTER(STAT_AI_Sense_Sight_ProcessPendingQuery);

	UE_MT_SCOPED_WRITE_ACCESS(QueriesListAccessDetector);

	const int32 QueryIdx = SightQueriesPending.IndexOfByPredicate([&QueryID](const FAICustomSightQuery& Element)
		{
...
void UAISense_CustomSight::OnPendingTraceQueryProcessed(const FTraceHandle& TraceHandle, FTraceDatum& TraceDatum)
{
	SCOPE_CYCLE_COUNTER(STAT_AI_Sense_Sight_ProcessPendingQuery);
	UE_MT_SCOPED_WRITE_ACCESS(QueriesListAccessDetector);

	const int32 QueryIdx = SightQueriesPending.IndexOfByPredicate([&TraceHandle](const FAICustomSightQuery& Element)
		{
	...
	if (const FAICustomSightTarget* Target = ObservedTargets.Find(SightQueriesPending[QueryIdx].TargetId))
	{
		TargetActor = Target->Target.Get();
	}
	const bool bIsVisible = UE::UAISense_CustomSight::IsTraceConsideredVisible(TraceDatum.OutHits.Num() > 0 ? &TraceDatum.OutHits[0] : nullptr, TargetActor);

	OnPendingQueryProcessed(QueryIdx, bIsVisible, DefaultStimulusStrength, TraceDatum.End, NullOpt, TargetActor);
}

void UAISense_CustomSight::OnPendingQueryProcessed(const int32 SightQueryIndex, const bool bIsVisible, const float StimulusStrength, const FVector& SeenLocation, const TOptional<int32>& UserData, const TOptional<AActor*> InTargetActor)
{
	FAICustomSightQuery SightQuery = SightQueriesPending[SightQueryIndex];
	...
	{
		const FAICustomSightTarget* Target = ObservedTargets.Find(SightQuery.TargetId);
...
void UAISense_CustomSight::RegisterEvent(const FAICustomSightEvent& Event)
{

}

void UAISense_CustomSight::RegisterSource(AActor& SourceActor)
{
	RegisterTarget(SourceActor);
}

void UAISense_CustomSight::UnregisterSource(AActor& SourceActor)
{
	UE_MT_SCOPED_WRITE_ACCESS(QueriesListAccessDetector);

	const FAICustomSightTarget::FTargetId AsTargetId = SourceActor.GetUniqueID();
	FAICustomSightTarget AsTarget;
		...
		&& (SightQueriesInRange.Num() + SightQueriesOutOfRange.Num()) > 0)
	{
		...
		if (TargetActor)
		{
			...
			// visible		
			AIPerception::FListenerMap& ListenersMap = *GetListeners();
			auto RemoveQuery = [this, &ListenersMap, &AsTargetId, &TargetActor](TArray<FAICustomSightQuery>& SightQueries, const int32 QueryIndex)->EReverseForEachResult
				{
					FAICustomSightQuery* SightQuery = &SightQueries[QueryIndex];
...
bool UAISense_CustomSight::RegisterTarget(AActor& TargetActor, const TFunction<void(FAICustomSightQuery&)>& OnAddedFunc /*= nullptr*/)
{
	SCOPE_CYCLE_COUNTER(STAT_AI_Sense_Sight_RegisterTarget);

	FAICustomSightTarget* SightTarget = ObservedTargets.Find(TargetActor.GetUniqueID());

	if (SightTarget != nullptr && SightTarget->GetTargetActor() != &TargetActor)
	{
		// this means given unique ID has already been recycled. 
		FAICustomSightTarget NewSightTarget(&TargetActor);
...
void UAISense_CustomSight::OnNewListenerImpl(const FPerceptionListener& NewListener)
{
	UAIPerceptionComponent* NewListenerPtr = NewListener.Listener.Get();
	check(NewListenerPtr);
	const UAISenseConfig_CustomSight* SenseConfig = Cast<const UAISenseConfig_CustomSight>(NewListenerPtr->GetSenseConfig(GetSenseID()));
	check(SenseConfig);
...
void UAISense_CustomSight::GenerateQueriesForListener(const FPerceptionListener& Listener, const FDigestedSightProperties& PropertyDigest, const TFunction<void(FAICustomSightQuery&)>& OnAddedFunc/*= nullptr */)
{
	bool bNewQueriesAdded = false;
	const IGenericTeamAgentInterface* ListenersTeamAgent = Listener.GetTeamAgent();
...
bool UAISense_CustomSight::RegisterNewQuery(const FPerceptionListener& Listener, const IGenericTeamAgentInterface* ListenersTeamAgent, const AActor& TargetActor, const FAICustomSightTarget::FTargetId& TargetId, const FVector& TargetLocation, const FDigestedSightProperties& PropDigest, const TFunction<void(FAICustomSightQuery&)>& OnAddedFunc)
{
	...
	FAICustomSightQuery& AddedQuery = bInRange ? SightQueriesInRange.AddDefaulted_GetRef() : SightQueriesOutOfRange.AddDefaulted_GetRef();
...
void UAISense_CustomSight::OnListenerUpdateImpl(const FPerceptionListener& UpdatedListener)
{
	...
	const FAICustomSightTarget::FTargetId AsTargetId = UpdatedListener.GetBodyActorUniqueID();
	FAICustomSightTarget* AsTarget = ObservedTargets.Find(AsTargetId);
	if (AsTarget != NULL)
	{
		if (AsTarget->Target.IsValid())
		{
			...
			RemoveAllQueriesToTarget(AsTargetId, [&LastVisibleObservers](const FAICustomSightQuery& Query)
				{
			...
			RegisterTarget(*(AsTarget->Target.Get()), [&LastVisibleObservers](FAICustomSightQuery& Query)
				{
	...
	if (UpdatedListener.HasSense(GetSenseID()))
	{
		...
		TSet<FAICustomSightTarget::FTargetId> LastVisibleTargets;
		RemoveAllQueriesByListener(UpdatedListener, [&LastVisibleTargets](const FAICustomSightQuery& Query)
			{
				if (Query.GetLastResult())
				{
		...
		const UAISenseConfig_CustomSight* SenseConfig = Cast<const UAISenseConfig_CustomSight>(UpdatedListener.Listener->GetSenseConfig(GetSenseID()));
		check(SenseConfig);
		...
		GenerateQueriesForListener(UpdatedListener, PropertiesDigest, [&LastVisibleTargets](FAICustomSightQuery& Query)
			{
...
void UAISense_CustomSight::OnListenerConfigUpdated(const FPerceptionListener& UpdatedListener)
{
	...
	if (PropertiesDigest)
	{
		...
		const UAISenseConfig_CustomSight* SenseConfig = CastChecked<const UAISenseConfig_CustomSight>(UpdatedListener.Listener->GetSenseConfig(GetSenseID()));
...
void UAISense_CustomSight::OnListenerRemovedImpl(const FPerceptionListener& RemovedListener)
{
...
void UAISense_CustomSight::RemoveAllQueriesByListener(const FPerceptionListener& Listener, const TFunction<void(const FAICustomSightQuery&)>& OnRemoveFunc/*= nullptr */)
{
	...
	auto RemoveQuery = [&ListenerId, &OnRemoveFunc](TArray<FAICustomSightQuery>& SightQueries, const int32 QueryIndex)->EReverseForEachResult
		{
			const FAICustomSightQuery& SightQuery = SightQueries[QueryIndex];
...
void UAISense_CustomSight::RemoveAllQueriesToTarget(const FAICustomSightTarget::FTargetId& TargetId, const TFunction<void(const FAICustomSightQuery&)>& OnRemoveFunc/*= nullptr */)
{
	SCOPE_CYCLE_COUNTER(STAT_AI_Sense_Sight_RemoveToTarget);
	UE_MT_SCOPED_WRITE_ACCESS(QueriesListAccessDetector);

	auto RemoveQuery = [&TargetId, &OnRemoveFunc](TArray<FAICustomSightQuery>& SightQueries, const int32 QueryIndex)->EReverseForEachResult
		{
			const FAICustomSightQuery& SightQuery = SightQueries[QueryIndex];
...
void UAISense_CustomSight::OnListenerForgetsActor(const FPerceptionListener& Listener, AActor& ActorToForget)
{
	const uint32 ListenerId = Listener.GetListenerID();
	const uint32 TargetId = ActorToForget.GetUniqueID();

	auto ForgetPreviousResult = [&ListenerId, &TargetId](FAICustomSightQuery& SightQuery)->EForEachResult
		{
...
void UAISense_CustomSight::OnListenerForgetsAll(const FPerceptionListener& Listener)
{
	UE_MT_SCOPED_WRITE_ACCESS(QueriesListAccessDetector);

	const uint32 ListenerId = Listener.GetListenerID();

	auto ForgetPreviousResult = [&ListenerId](FAICustomSightQuery& SightQuery)->EForEachResult
		{
			if (SightQuery.ObserverId == ListenerId)
			{
				SightQuery.ForgetPreviousResult();
